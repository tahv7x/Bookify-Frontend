...
import { Mail, ArrowRight, Sun, Moon, ArrowLeft } from "lucide-react";
...
const ForgotPassword: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessMessage("Lien envoyé ! Redirection...");
      ...
      <div className="relative z-10 w-full max-w-[480px] px-6">
        <div className="bg-white/80 dark:bg-[#1a1d27]/90 backdrop-blur-3xl border border-white/60 dark:border-white/10 rounded-[2.5rem] p-8 md:p-12 shadow-[0_32px_64px_-16px_rgba(0,0,0,0.08)] dark:shadow-[0_32px_64px_-16px_rgba(0,0,0,0.6)] overflow-hidden transition-all duration-500">
          ...
          <div className="text-center mb-10">
            <h1 className="text-3xl md:text-4xl font-fraunces font-bold text-slate-900 dark:text-white mb-3">Mot de passe oublié</h1>
            <p className="text-slate-500 dark:text-slate-400 text-sm">Entrez votre email pour recevoir un lien de réinitialisation</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            >
              {loading ? "Envoi..." : (
                <>
                  Envoyer le lien <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
                </>
              )}
            </button>

            <div className="text-center mt-8">
              <Link to="/login" className="inline-flex items-center gap-2 text-[#1A6FD1] font-extrabold hover:underline underline-offset-4 decoration-2">
