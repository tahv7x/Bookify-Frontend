    ...
    <div className="min-h-screen bg-[#F4F7FE] dark:bg-dark-bg transition-colors duration-200">
      <style>{`
        ...
        * { font-family:'Poppins',-apple-system,BlinkMacSystemFont,sans-serif; }
        ...
        .apt-row{animation:slideInUp .5s cubic-bezier(.16,1,.3,1) both; transition:transform .2s ease;}
        .apt-row:nth-child(1){animation-delay:.30s} .apt-row:nth-child(2){animation-delay:.40s}
        .apt-row:nth-child(3){animation-delay:.50s} .apt-row:nth-child(4){animation-delay:.60s}
        .apt-row:hover{transform:translateX(4px);}
        .hist-item{animation:slideInRight .5s cubic-bezier(.16,1,.3,1) both; transition:all .2s ease;}
        .hist-item:nth-child(1){animation-delay:.70s}
        .hist-item:nth-child(2){animation-delay:.80s}
        .hist-item:nth-child(3){animation-delay:.90s}
        .hist-item:hover{transform:scale(1.02);}
      ...
      <main className="min-h-screen">
        ...
        <div className="p-4 sm:p-6 lg:p-8 space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="stat-card"><StatCard icon={Calendar}    iconBg="bg-blue-100 dark:bg-blue-500/15"    iconColor="text-[#0059B2] dark:text-blue-400"    label="Total Rendez-vous"     value="17" trend="+3" trendUp={true}/></div>
            <div className="stat-card"><StatCard icon={CheckCircle} iconBg="bg-emerald-100 dark:bg-emerald-500/15" iconColor="text-emerald-600 dark:text-emerald-400" label="Rendez-vous confirmés" value="12" trend="+1" trendUp={true}/></div>
            <div className="stat-card"><StatCard icon={Clock}       iconBg="bg-orange-100 dark:bg-orange-500/15" iconColor="text-orange-500 dark:text-orange-400"   label="En attente"            value="5"  trend="-2" trendUp={false}/></div>
          </div>

          {/* ROW 1: Appointments + Right widgets */}
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 bg-white dark:bg-dark-surface rounded-2xl p-4 sm:p-6 shadow-sm border border-gray-100 dark:border-dark-border transition-colors duration-200 anim-up" style={{animationDelay:'.1s'}}>
              ...
              <div className="space-y-2">
                ...
                {appointments.map((apt, idx) => (
                  <div key={idx} className="apt-row bg-gray-50 dark:bg-dark-bg rounded-xl p-3 sm:p-4">
                    {/* Mobile */}
                    <div className="sm:hidden space-y-2">
                      <div className="flex items-start justify-between">
                        <div>
                          <p className="font-semibold text-gray-900 dark:text-dark-text text-sm">{apt.name}</p>
                          <p className="text-xs text-gray-500 dark:text-dark-muted">{apt.specialty}</p>
                        </div>
                      ...
                      <div className="flex justify-between text-xs text-gray-500 dark:text-dark-muted">
                        <span>{apt.date}</span><span className="font-medium">{apt.time}</span>
                      </div>
                    </div>
                    {/* Desktop */}
            ...
            <div className="space-y-5">
              ...
              <div className="bg-white dark:bg-dark-surface rounded-2xl p-5 shadow-sm border border-gray-100 dark:border-dark-border transition-colors duration-200 anim-up" style={{animationDelay:'.15s'}}>
                ...
                <div className="bg-gray-50 dark:bg-dark-bg rounded-xl p-6 text-center border-2 border-dashed border-gray-200 dark:border-dark-border hover:border-[#0059B2] dark:hover:border-blue-500 transition-all cursor-pointer group">
                  <div className="w-12 h-12 bg-blue-100 dark:bg-blue-500/20 rounded-2xl flex items-center justify-center mx-auto mb-3 float-icon">
                    <Upload className="text-[#0059B2] dark:text-blue-400" size={22}/>
                  </div>
                ...
                <button className="w-full mt-3 bg-gradient-to-r from-[#004a96] to-[#1A6FD1] hover:from-[#003d80] hover:to-[#1560b8] text-white py-2.5 rounded-xl font-semibold text-sm transition-all shadow-md hover:shadow-lg">
                  Poster
                </button>
              </div>
              ...
              <div className="bg-white dark:bg-dark-surface rounded-2xl p-5 shadow-sm border border-gray-100 dark:border-dark-border transition-colors duration-200 anim-up" style={{animationDelay:'.2s'}}>
                ...
                <div className="relative w-40 h-40 mx-auto mb-4">
                  <svg viewBox="0 0 200 200" className="transform -rotate-90 w-full h-full">
                    <circle cx="100" cy="100" r="70" fill="none" stroke="#1A6FD1" strokeWidth="36" strokeDasharray="321 440" strokeDashoffset="0"/>
                ...
                <div className="space-y-2">
                  {[{color:'bg-[#1A6FD1]',label:'Santé Médicale',val:'73%'},{color:'bg-slate-600',label:'Beauté & Bien-être',val:'16%'},{color:'bg-[#0c5a7c]',label:'Services tech.',val:'11%'}].map((item,i)=>(
                    <div key={i} className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <div className={`w-2.5 h-2.5 rounded-full ${item.color}`}/>
                        <span className="text-xs text-gray-600 dark:text-dark-text">{item.label}</span>
                      </div>
            ...
            <div className="lg:col-span-2 bg-white dark:bg-dark-surface rounded-2xl p-5 sm:p-6 shadow-sm border border-gray-100 dark:border-dark-border transition-colors duration-200 anim-up" style={{animationDelay:'.25s'}}>
              <div className="flex items-center justify-between mb-5">
                <h3 className="text-base font-bold text-gray-900 dark:text-dark-text flex items-center gap-2">
                  <Activity size={16} className="text-[#0059B2]"/>Spécialistes recommandés
                </h3>
                <button className="text-xs text-[#0059B2] dark:text-blue-400 font-semibold hover:underline">Voir tout</button>
              </div>
              <div className="grid sm:grid-cols-3 gap-3">
                {specialists.map((sp, i) => {
                  const SpIcon = sp.icon;
                  return (
                    <div key={i} className="bg-gray-50 dark:bg-dark-bg rounded-xl p-4 flex flex-col items-center text-center hover:shadow-md transition-all cursor-pointer border border-transparent hover:border-blue-100 dark:hover:border-dark-border">
                      ...
                      <div className="w-full bg-blue-50 dark:bg-blue-500/10 rounded-lg px-2 py-1.5">
                        <p className="text-[10px] text-[#0059B2] dark:text-blue-400 font-semibold">{sp.next}</p>
                      </div>
                    </div>
                  );
                })}
              ...
              <div className="bg-white dark:bg-dark-surface rounded-2xl p-5 shadow-sm border border-gray-100 dark:border-dark-border transition-colors duration-200 anim-right" style={{animationDelay:'.3s'}}>
                ...
                <div className="space-y-2 mb-4">
                  {recentHistory.map((item, idx) => (
                    <div key={idx} className="hist-item flex items-center gap-3 p-2.5 bg-gray-50 dark:bg-dark-bg rounded-xl hover:bg-gray-100 dark:hover:bg-dark-border cursor-pointer">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-gray-500 dark:text-dark-muted">{item.specialty} · <span className="text-[#0059B2]">{item.time}</span></p>
                      </div>
                    </div>
                  ))}
                </div>
                <button className="w-full bg-gradient-to-r from-[#004a96] to-[#1A6FD1] hover:from-[#003d80] hover:to-[#1560b8] text-white py-2.5 rounded-xl font-semibold text-sm transition-all">
                  Voir plus
                </button>
              </div>
