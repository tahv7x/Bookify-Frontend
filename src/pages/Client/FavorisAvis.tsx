...
const FavorisAvis: React.FC = () => {
    ...
    const fetchReviews = async () => {
      try {
        ...
        if (s) {
          ...
          if (clientId) {
            const { getAvisByClient } = await import('../../services/Client/avisService');
            ...
            if (data) {
              const mappedReviews = data.map((r: any) => ({
                ...
                providerImage: 'https://ui-avatars.com/api/?name=' + r.providerName,
                date: r.date,
                rating: r.rating,
                text: r.comment,
                aspects: { welcome: r.rating, cleanliness: r.rating, quality: r.rating, value: r.rating }
              }));
  ...
  const handleDeleteReview = async (reviewId: string) => {
    if (window.confirm("Voulez-vous vraiment supprimer cet avis ?")) {
      try {
        const { deleteAvis } = await import('../../services/Client/avisService');
  ...
  const handleSaveReview = () => {
    ...
    setReviews(prev => prev.map(rev =>
      rev.id === editingReview.id
        ? { ...rev, text: editingText, rating: editingRating, aspects: editingAspects, date: "Modifié aujourd'hui" }
        : rev
    ));
  ...
  const handleOpenBooking = async (prof: Professional) => {
    ...
    try {
      ...
      const { getDisponibilites } = await import('../../services/provider/disponibiliteService');
  ...
  const handleConfirmBooking = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!bookingProfessional || !selectedService || !selectedDate || !selectedTimeSlot) return;

    try {
      const { createRendezVous } = await import('../../services/Client/rendezVousService');
  ...
  const availableSlotsForDate = React.useMemo(() => {
    ...
    return dayData.slots.filter((s: any) => s.available).map((s: any) => s.time);
  }, [selectedDate, providerAvailability]);

  return (
    <div
      className="min-h-screen transition-colors duration-200 overflow-x-hidden"
      style={{ fontFamily: "'Inter', sans-serif", background: isDark ? '#0d1117' : '#eef2fc' }}
    >
      <style>{css}</style>
      ...
      <main className="min-h-screen relative bg-dot-pattern pb-20 md:pb-0">
        ...
        <div className="max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6 relative z-10">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
            <div>
              <h1 className="font-fraunces text-2xl sm:text-3xl font-bold tracking-tight mb-1"
                style={{ color: isDark ? '#f1f5f9' : '#1a2540' }}>
                Mes Favoris & Avis
              </h1>
              <p className="text-sm" style={{ color: isDark ? 'rgba(255,255,255,0.38)' : '#94a3b8' }}>
            ...
            <div className="glass-card rounded-3xl p-5 flex items-center justify-between overflow-hidden relative group">
              ...
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-2xl stat-icon-red flex items-center justify-center">
                  <Heart className="text-red-500" size={22} fill="rgba(239,68,68,0.15)" />
                </div>
                <div>
                  <span className="font-fraunces text-2xl font-bold" style={{ color: isDark ? '#f1f5f9' : '#1a2540' }}>
                    {favorites.length}
                  </span>
                  <p className="text-xs font-medium" style={{ color: isDark ? 'rgba(255,255,255,0.38)' : '#94a3b8' }}>
                    Favoris enregistrés
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <div className="flex items-baseline gap-1">
                    <span className="font-fraunces text-2xl font-bold" style={{ color: isDark ? '#f1f5f9' : '#1a2540' }}>{averageRating}</span>
                    <span className="text-xs" style={{ color: isDark ? 'rgba(255,255,255,0.38)' : '#94a3b8' }}>/5.0</span>
                  </div>
                  <p className="text-xs font-medium" style={{ color: isDark ? 'rgba(255,255,255,0.38)' : '#94a3b8' }}>
                    Note moyenne donnée
                  </p>
          ...
          {/* ── Tab navigation ── */}
          <div className="flex items-center justify-between mb-8 pb-px"
            style={{ borderBottom: `1px solid ${isDark ? 'rgba(255,255,255,0.07)' : 'rgba(26,111,209,0.09)'}` }}>
              ...
              >
                ...
                {activeTab === 'favorites' && (
                  <motion.div
                    ...
                    style={{ background: '#1a6fd1', boxShadow: '0 0 8px rgba(26,111,209,0.6)' }}
                  />
                )}
              </button>
              <button
          ...
          <AnimatePresence mode="wait">
            {loading ? (
              ...
              >
                {[1, 2, 3, 4].map(i => (
                  <div key={i} className="glass-card rounded-3xl p-6 flex flex-col sm:flex-row justify-between gap-5 h-[180px]">
                ...
                {filteredFavorites.length > 0 ? (
                      ...
                      >
                        <div className="flex gap-4">
                          <div className="relative w-20 h-20 rounded-2xl overflow-hidden shrink-0"
                            style={{ border: `1px solid ${isDark ? 'rgba(255,255,255,0.09)' : 'rgba(200,215,255,0.5)'}` }}>
                            <img
                              src={prof.image} alt={prof.name}
                          ...
                          <div>
                            ...
                            <h3 className="font-fraunces font-bold text-lg mt-1.5"
                              style={{ color: isDark ? '#f1f5f9' : '#1a2540' }}>
                              {prof.name}
                            </h3>
                            <div className="flex items-center gap-1.5 mt-1">
                              <Star size={13} className="text-amber-400" fill="currentColor" />
                              <span className="text-xs font-bold" style={{ color: isDark ? '#f1f5f9' : '#1a2540' }}>
                                {prof.rating}
                              </span>
                              <span className="text-xs" style={{ color: isDark ? 'rgba(255,255,255,0.38)' : '#94a3b8' }}>
                        ...
                        <div className="flex sm:flex-col justify-between items-end gap-3 shrink-0 pt-4 sm:pt-0"
                          style={{ borderTop: `1px solid ${isDark ? 'rgba(255,255,255,0.06)' : 'rgba(26,111,209,0.07)'}` }}
                        >
                          <button
                  ...
                  >
                    <div className="w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-4"
                      style={{ background: isDark ? 'rgba(239,68,68,0.1)' : 'rgba(254,242,242,1)' }}>
                      <HeartOff className="text-red-500" size={28} />
                    </div>
                    <h3 className="font-fraunces text-lg font-bold mb-2" style={{ color: isDark ? '#f1f5f9' : '#1a2540' }}>
                      Aucun favori trouvé
                    </h3>
                    <p className="text-sm leading-relaxed" style={{ color: isDark ? 'rgba(255,255,255,0.38)' : '#94a3b8' }}>
                      {searchTerm
              ...
              >
                {filteredReviews.length > 0 ? (
                      ...
                      >
                        <div className="flex-1">
                          <div className="flex items-center gap-3.5 mb-4">
                            <img
                              src={rev.providerImage} alt={rev.providerName}
                              className="w-11 h-11 rounded-full object-cover"
                              style={{ border: `1.5px solid ${isDark ? 'rgba(255,255,255,0.09)' : 'rgba(200,215,255,0.6)'}` }}
                            />
                            <div>
                              <h3 className="font-bold text-sm" style={{ color: isDark ? '#f1f5f9' : '#1a2540' }}>
                                {rev.providerName}
                              </h3>
                              <p className="text-xs" style={{ color: isDark ? 'rgba(255,255,255,0.38)' : '#94a3b8' }}>
                          ...
                          <div className="flex items-center gap-2 mb-3">
                            <div className="flex gap-0.5">
                              {[1, 2, 3, 4, 5].map(s => (
                                <Star key={s} size={14}
                                  className={s <= rev.rating ? 'text-amber-400' : 'text-gray-200 dark:text-gray-700'}
                          ...
                          {rev.aspects && (
                            <div className="mt-4 pt-3 grid grid-cols-2 sm:grid-cols-4 gap-4"
                              style={{ borderTop: `1px solid ${isDark ? 'rgba(255,255,255,0.06)' : 'rgba(26,111,209,0.07)'}` }}>
                              {[
                                { label: 'Accueil', val: rev.aspects.welcome },
                                { label: 'Propreté', val: rev.aspects.cleanliness },
                                { label: 'Qualité', val: rev.aspects.quality },
                                { label: 'Rapport Q/P', val: rev.aspects.value },
                              ].map(({ label, val }) => (
                                <div key={label}>
                                  ...
                                  <div className="flex items-center gap-1.5">
                                    ...
                                    <div className="h-1.5 flex-1 rounded-full overflow-hidden"
                                      style={{ background: isDark ? 'rgba(255,255,255,0.06)' : 'rgba(26,111,209,0.08)' }}>
                                      <div className="h-full rounded-full bg-emerald-500" style={{ width: `${(val / 5) * 100}%` }} />
                                    </div>
                                  </div>
                                </div>
                              ))}
                        ...
                        <div className="flex md:flex-col justify-end items-end gap-3 shrink-0 pt-4 md:pt-0"
                          style={{ borderTop: `1px solid ${isDark ? 'rgba(255,255,255,0.06)' : 'rgba(26,111,209,0.07)'}` }}>
                          <button
                  ...
                  >
                    <div className="w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-4"
                      style={{ background: isDark ? 'rgba(26,111,209,0.1)' : 'rgba(239,246,255,1)' }}>
      ...
      <AnimatePresence>
        {showToast && (
          <motion.div
            ...
            style={{
              background: isDark ? 'rgba(13,17,27,0.95)' : 'rgba(15,23,42,0.92)',
          ...
          >
            ...
            <span className="text-sm font-medium text-white">Favori retiré.</span>
            <button
      ...
      <AnimatePresence>
        {bookingProfessional && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
            style={{ background: 'rgba(10,14,22,0.72)', backdropFilter: 'blur(12px)' }}>
            <motion.div
            ...
            >
              ...
              <div className="p-6 flex justify-between items-center"
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-bold text-base" style={{ color: isDark ? '#f1f5f9' : '#1a2540' }}>
                      {bookingProfessional.name}
                    </h3>
                    <p className="text-xs" style={{ color: isDark ? 'rgba(255,255,255,0.38)' : '#94a3b8' }}>
                      {bookingProfessional.specialty} · {bookingProfessional.city}
                    </p>
                  </div>
                </div>
                <button
                  ...
                  style={{
                    color: isDark ? 'rgba(255,255,255,0.38)' : '#94a3b8',
                    background: isDark ? 'rgba(255,255,255,0.05)' : 'rgba(26,111,209,0.05)'
                  }}
                >
                  <X size={17} />
                </button>
              ...
              <form onSubmit={handleConfirmBooking} className="p-6 space-y-5">
                {isBookingSuccess ? (
                  ...
                  >
                    <div className="w-16 h-16 rounded-full flex items-center justify-center mb-4 text-emerald-500"
                      style={{ background: isDark ? 'rgba(34,197,94,0.12)' : 'rgba(220,252,231,1)', boxShadow: '0 0 0 8px rgba(34,197,94,0.08)' }}>
                      <Check size={34} strokeWidth={2.5} className="animate-pulse" />
                    </div>
                    <h4 className="font-fraunces text-lg font-bold mb-1" style={{ color: isDark ? '#f1f5f9' : '#1a2540' }}>
                      Réservation confirmée !
                    </h4>
                    <p className="text-xs text-center max-w-xs leading-relaxed" style={{ color: isDark ? 'rgba(255,255,255,0.38)' : '#94a3b8' }}>
                  ...
                  <>
                    <div>
                      ...
                      <select
                        required value={selectedService} onChange={e => setSelectedService(e.target.value)}
                        className="glass-input w-full px-4 py-3 rounded-2xl text-sm"
                        style={{ color: isDark ? '#f1f5f9' : '#1a2540' }}
                      >
                        <option value="">Sélectionner une prestation</option>
                        {providerServices.map(s => (
                          <option key={s.id} value={s.id}>{s.name || s.nom} ({s.prix} MAD)</option>
                        ))}
                      </select>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="relative">
                        <Calendar size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2" style={{ color: isDark ? 'rgba(255,255,255,0.3)' : '#aab8d0' }} />
                        <input
                    ...
                    <div>
                      ...
                      {!selectedDate ? (
                        <p className="text-sm italic text-gray-500 py-2">Veuillez d'abord sélectionner une date.</p>
                      ) : availableSlotsForDate.length === 0 ? (
                        <p className="text-sm italic text-red-500 py-2">Aucun créneau disponible pour cette date.</p>
                      ) : (
                        <div className="grid grid-cols-4 gap-2">
                          {availableSlotsForDate.map((slot: string) => (
                            <button
                              key={slot} type="button" onClick={() => setSelectedTimeSlot(slot)}
                              className="py-2 px-1 text-center rounded-xl text-xs font-bold transition-all"
                              style={
                                selectedTimeSlot === slot
                                  ? { background: 'linear-gradient(135deg, #2176d8, #0f4fa0)', color: '#fff', boxShadow: '0 4px 12px rgba(26,111,209,0.32)', border: '1px solid transparent' }
                                  : { background: isDark ? 'rgba(255,255,255,0.05)' : 'rgba(240,245,255,0.95)', color: isDark ? '#f1f5f9' : '#1a2540', border: `1px solid ${isDark ? 'rgba(255,255,255,0.09)' : 'rgba(190,210,255,0.5)'}` }
                              }
                            >
                              {slot}
                            </button>
                    ...
                    <div className="flex gap-3 pt-4"
                      style={{ borderTop: `1px solid ${isDark ? 'rgba(255,255,255,0.07)' : 'rgba(26,111,209,0.09)'}` }}>
                      <button
                        type="button" onClick={() => setBookingProfessional(null)}
                        className="flex-1 py-3 rounded-2xl text-sm font-semibold transition-all"
                        style={{
                          color: isDark ? 'rgba(255,255,255,0.5)' : '#64748b',
                          border: `1px solid ${isDark ? 'rgba(255,255,255,0.09)' : 'rgba(190,210,255,0.5)'}`,
                          background: isDark ? 'rgba(255,255,255,0.04)' : 'rgba(240,245,255,0.6)',
                        }}
                      >
                        Annuler
                      </button>
                      <button
                        ...
                        style={{ background: 'linear-gradient(135deg, #2176d8, #0f4fa0)', boxShadow: '0 4px 20px rgba(26,111,209,0.32)' }}
                      >
              ...
              <div className="p-6 space-y-5 sba max-h-[75vh] overflow-y-auto">
                ...
                <div>
                  ...
                  <div className="flex gap-1.5 items-center">
                    {[1, 2, 3, 4, 5].map(v => (
                      <button key={v} onClick={() => setEditingRating(v)} type="button"
                        className="transition-transform active:scale-95">
                        <Star size={28}
                          className={v <= editingRating ? 'text-amber-400' : 'text-gray-200 dark:text-gray-700'}
                          fill={v <= editingRating ? 'currentColor' : 'none'}
                        />
                      </button>
                    ))}
                    <span className="text-sm font-bold ml-2" style={{ color: isDark ? '#f1f5f9' : '#1a2540' }}>
                      {editingRating} sur 5
                    </span>
                  </div>
                </div>
                ...
                <div className="space-y-3 pt-3"
                  ...
                  {[
                    ...
                    { key: 'cleanliness' as const, label: 'Propreté & Hygiène' },
                    { key: 'quality' as const, label: 'Qualité de la prestation' },
                    { key: 'value' as const, label: 'Rapport qualité / prix' },
                  ].map(({ key, label }) => (
                    <div key={key} className="flex items-center justify-between">
                      <span className="text-sm" style={{ color: isDark ? 'rgba(255,255,255,0.6)' : '#475569' }}>
                        {label}
                      </span>
                      <div className="flex gap-1">
                        {[1, 2, 3, 4, 5].map(v => (
                          <button
                            key={v} type="button"
                            onClick={() => setEditingAspects(prev => ({ ...prev, [key]: v }))}
                            className="w-7 h-7 text-xs font-bold rounded-lg flex items-center justify-center transition-all"
                            style={
                              editingAspects[key] === v
                                ? { background: '#22c55e', color: '#fff', border: '1px solid transparent' }
                                : {
                                  background: isDark ? 'rgba(255,255,255,0.05)' : 'rgba(240,245,255,0.95)',
                                  color: isDark ? 'rgba(255,255,255,0.5)' : '#64748b',
                                  border: `1px solid ${isDark ? 'rgba(255,255,255,0.09)' : 'rgba(190,210,255,0.5)'}`,
                                }
                            }
                ...
                <div>
                  ...
                  <textarea
                    rows={4} value={editingText} onChange={e => setEditingText(e.target.value)}
                ...
                <div className="flex gap-3 pt-4"
                  ...
                  <button
                    type="button" onClick={handleSaveReview} disabled={!editingText.trim()}
