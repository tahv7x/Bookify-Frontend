...
import { Search, Send, Check, CheckCheck, MoreVertical, Smile, AlertCircle, ChevronLeft, MessageSquare, Loader2 } from 'lucide-react';
...
import { getContacts, getConversation, sendMessage, type Contact } from '../../services/Client/messageService';
...
const Messages: React.FC = () => {
  ...
  const loadMessagesSilentlyRef = useRef<((contactId: number) => Promise<void>) | null>(null);
  ...
  useEffect(() => {
    if (activeContactId) {
      ...
      loadMessagesSilentlyRef.current?.(activeContactId).finally(() => setLoadingMessages(false));
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessages(prev => prev.map(m =>
        m.id === tempId
          ? { ...m, id: res.id, time: new Date(res.sentAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }), pending: false }
          : m
      ));
      setContacts(prev => prev.map(c =>
        c.id === activeContactId ? { ...c, lastMessage: text, time: newMessage.time } : c
      ));
    } catch (err) {
      console.error(err);
      // Mark as failed
  ...
  const avatarEl = (name: string, specialty: string, size = 'w-11 h-11') =>
    <div className={`${size} rounded-full flex items-center justify-center text-white font-bold text-base ${specialty === 'Administration' ? 'bg-gradient-to-br from-gray-600 to-gray-800' : 'bg-gradient-to-br from-[#2176d8] to-[#0f4fa0]'}`}
      style={{ boxShadow: '0 2px 12px rgba(26,111,209,0.28)', flexShrink: 0 }}>
      {specialty === 'Administration' ? <AlertCircle size={18} /> : name.charAt(0).toUpperCase()}
    </div>;

  return (
    <div className="h-screen flex flex-col overflow-hidden" style={{ fontFamily: "'Inter', sans-serif", background: isDark ? '#0d1117' : '#eef2fc' }}>
      <style>{css}</style>
      ...
      <div className={`fixed left-0 top-0 h-full w-64 bg-transparent transform transition-transform duration-300 ease-out z-50 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        {role === 'PRESTATAIRE'
          ? <ProviderNavbar activeSection="messages" onSectionChange={() => setIsSidebarOpen(false)} />
          : <Navbar activeSection="messages" onSectionChange={() => setIsSidebarOpen(false)} />}
      </div>

      {/* Main */}
      <div className="flex-1 flex flex-col min-h-0 relative z-10 dot-bg">
        {role === 'PRESTATAIRE'
          ? <ProviderTopBar userName={userName} onMenuToggle={() => setIsSidebarOpen(v => !v)} isMobileMenuOpen={isSidebarOpen} />
          : <TopBar userName={userName} onMenuToggle={() => setIsSidebarOpen(v => !v)} isMobileMenuOpen={isSidebarOpen} />}

        <div className="flex-1 min-h-0 p-3 md:p-5 flex gap-3 md:gap-4 pb-20 md:pb-5" style={{ height: 'calc(100% - 64px)' }}>

          {/* ═══ Contacts sidebar ═══ */}
          <div className={`gls rounded-2xl w-full md:w-72 lg:w-80 flex-shrink-0 flex flex-col overflow-hidden ${activeContactId ? 'hidden md:flex' : 'flex'}`}>

            {/* Header */}
            <div className="px-4 pt-4 pb-3 flex-shrink-0" style={{ borderBottom: `1px solid ${isDark ? 'rgba(255,255,255,0.06)' : 'rgba(26,111,209,0.07)'}` }}>
              <h1 className="ff text-lg font-bold mb-3" style={{ color: isDark ? '#f1f5f9' : '#1a2540' }}>Discussions</h1>
              <div className="relative">
                <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2" style={{ color: isDark ? 'rgba(255,255,255,0.3)' : '#aab8d0' }} />
            ...
            <div className="flex-1 overflow-y-auto sba p-2 space-y-0.5">
              {loadingContacts ? (
                ...
                <div className="flex flex-col items-center justify-center py-12 opacity-50">
                  <MessageSquare size={30} style={{ color: isDark ? '#94a3b8' : '#94a3b8' }} />
                  <p className="mt-2 text-xs" style={{ color: isDark ? 'rgba(255,255,255,0.35)' : '#94a3b8' }}>Aucun contact</p>
                </div>
              ) : filteredContacts.map(contact => (
                <motion.div key={contact.id} whileTap={{ scale: 0.98 }}
                  ...
                  <div className="relative shrink-0">
                    {contact.avatar
                      ? <img src={contact.avatar} alt={contact.name} className="w-11 h-11 rounded-full object-cover" style={{ boxShadow: '0 2px 8px rgba(0,0,0,0.18)' }} />
                      : avatarEl(contact.name, contact.specialty)}
                    {contact.isOnline && <div className="odot absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full" />}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-center mb-0.5">
                      <p className="font-semibold text-sm truncate" style={{ color: activeContactId === contact.id ? '#2176d8' : (isDark ? '#f1f5f9' : '#1a2540') }}>
                        {contact.name}
                      </p>
                      <span className="text-[10px] whitespace-nowrap ml-1" style={{ color: contact.unread > 0 ? '#2176d8' : (isDark ? 'rgba(255,255,255,0.28)' : '#aab8d0'), fontWeight: contact.unread > 0 ? 600 : 400 }}>
                        {contact.time}
                      </span>
                    </div>
                    <p className="text-xs truncate" style={{ color: contact.unread > 0 ? (isDark ? '#f1f5f9' : '#1a2540') : (isDark ? 'rgba(255,255,255,0.38)' : '#94a3b8'), fontWeight: contact.unread > 0 ? 500 : 400 }}>
                      {contact.lastMessage}
                    </p>
                  </div>
                  {contact.unread > 0 && (
                    <div className="shrink-0 w-5 h-5 rounded-full bg-[#2176d8] flex items-center justify-center text-[10px] font-bold text-white" style={{ boxShadow: '0 2px 8px rgba(26,111,209,0.38)' }}>
                      {contact.unread}
                    </div>
                  )}
                </motion.div>
          ...
          {/* ═══ Chat window ═══ */}
          <div className={`gls rounded-2xl flex-1 min-w-0 flex flex-col overflow-hidden ${!activeContactId ? 'hidden md:flex' : 'flex'}`}>
            {!activeContact ? (
              <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
                <div className="w-20 h-20 rounded-2xl flex items-center justify-center mb-5"
                  style={{ background: isDark ? 'rgba(26,111,209,0.1)' : 'rgba(26,111,209,0.07)', border: `1px solid ${isDark ? 'rgba(26,111,209,0.22)' : 'rgba(26,111,209,0.14)'}` }}>
                  <Send size={30} style={{ color: '#2176d8', marginLeft: 3 }} />
                </div>
                <h2 className="ff text-lg font-bold mb-1.5" style={{ color: isDark ? '#f1f5f9' : '#1a2540' }}>Vos messages</h2>
                <p className="text-sm max-w-xs leading-relaxed" style={{ color: isDark ? 'rgba(255,255,255,0.38)' : '#94a3b8' }}>
                  {role === 'PRESTATAIRE' ? 'Sélectionnez une conversation pour discuter avec vos clients.' : 'Sélectionnez une conversation pour discuter avec vos prestataires.'}
                </p>
              </div>
            ) : (
              <>
                ...
                <div className="hd px-4 md:px-5 py-3 flex items-center justify-between flex-shrink-0">
                  <div className="flex items-center gap-3">
                    <button onClick={() => setActiveContactId(null)} className="md:hidden p-1.5 rounded-xl" style={{ color: isDark ? 'rgba(255,255,255,0.45)' : '#64748b' }}>
                      <ChevronLeft size={22} />
                    </button>
                    <div className="relative shrink-0">
                      {activeContact.avatar
                        ? <img src={activeContact.avatar} alt={activeContact.name} className="w-10 h-10 rounded-full object-cover" style={{ boxShadow: '0 2px 8px rgba(0,0,0,0.18)' }} />
                        : avatarEl(activeContact.name, activeContact.specialty, 'w-10 h-10')}
                      {activeContact.isOnline && <div className="odot absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full" />}
                    </div>
                    <div
                      className={activeContact.specialty !== 'Administration' && activeContact.specialty !== 'Client' && activeContact.providerId ? 'cursor-pointer' : ''}
                      onClick={() => {
                        if (activeContact.specialty !== 'Administration' && activeContact.specialty !== 'Client' && activeContact.providerId)
                          navigate(`/Service-Provider-Profile/${activeContact.providerId}`);
                      }}>
                      <h2 className="font-bold text-sm md:text-base leading-tight" style={{ color: isDark ? '#f1f5f9' : '#1a2540' }}>{activeContact.name}</h2>
                      <div className="flex items-center gap-2">
                        <p className="text-xs font-medium" style={{ color: '#2176d8' }}>
                          {activeContact.specialty === 'Administration' ? 'Support Officiel' : activeContact.specialty}
                        </p>
                  ...
                  <button className="w-8 h-8 rounded-xl flex items-center justify-center transition-all"
                    style={{ color: isDark ? 'rgba(255,255,255,0.38)' : '#94a3b8', background: isDark ? 'rgba(255,255,255,0.05)' : 'rgba(26,111,209,0.05)' }}>
                    <MoreVertical size={17} />
                  </button>
                </div>

                {/* Messages */}
                <div ref={messagesAreaRef} className="flex-1 overflow-y-auto sba px-4 md:px-6 py-4">
                  {/* Date separator */}
                  <div className="flex items-center gap-3 mb-4">
                    <div className="flex-1 h-px" style={{ background: isDark ? 'rgba(255,255,255,0.06)' : 'rgba(26,111,209,0.08)' }} />
                    <span className="text-[10px] font-semibold uppercase tracking-widest px-3 py-1 rounded-full"
                      style={{ color: isDark ? 'rgba(255,255,255,0.3)' : '#94a3b8', background: isDark ? 'rgba(255,255,255,0.05)' : 'rgba(26,111,209,0.05)' }}>
                      Aujourd'hui
                    </span>
                    <div className="flex-1 h-px" style={{ background: isDark ? 'rgba(255,255,255,0.06)' : 'rgba(26,111,209,0.08)' }} />
                  </div>
                      ...
                      {[1, 2, 3, 4].map(i => {
                        ...
                        return (
                          <div key={i} className={`flex items-end gap-2 ${isMe ? 'justify-end' : 'justify-start'}`}>
                            {!isMe && <div className="sk w-8 h-8 rounded-full shrink-0" />}
                            <div className={`flex flex-col ${isMe ? 'items-end' : 'items-start'} gap-1`}>
                              <div className={`sk rounded-2xl h-10 ${isMe ? 'w-44' : 'w-60'}`} />
                      ...
                      {messages.map((msg, index) => {
                        ...
                        return (
                          <motion.div
                            ...
                            style={{ marginBottom: isLastInGroup ? '12px' : '3px' }}>
                            ...
                            {!isMe && (
                              <div className="w-8 shrink-0">
                                {showAvatar && (
                                  activeContact.avatar
                                    ? <img src={activeContact.avatar} alt="Avatar" className="w-8 h-8 rounded-full object-cover" />
                                    : <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-bold ${activeContact.specialty === 'Administration' ? 'bg-gray-700' : 'bg-gradient-to-br from-[#2176d8] to-[#0f4fa0]'}`}>
                                      {activeContact.specialty === 'Administration' ? 'A' : activeContact.name.charAt(0).toUpperCase()}
                                    </div>
                                )}
                              </div>
                            )}
                            ...
                            {/* Bubble */}
                            <div className={`flex flex-col ${isMe ? 'items-end' : 'items-start'} max-w-[70%] md:max-w-[58%]`}>
                              <div className={`px-4 py-2.5 text-sm leading-relaxed ${isMe ? `bm${msg.pending ? ' bm-pending' : msg.failed ? ' bm-failed' : ''}` : 'bo'}`}>
                                {msg.text}
                              </div>
                              {isLastInGroup && (
                                <div className="flex items-center gap-1.5 mt-1 px-1">
                                  <span className="text-[10px] font-medium" style={{ color: isDark ? 'rgba(255,255,255,0.28)' : '#b0c0d8' }}>{msg.time}</span>
                                  {isMe && (
                                    msg.pending
                                      ? <Loader2 size={12} className="animate-spin" style={{ color: isDark ? 'rgba(255,255,255,0.3)' : '#b0c0d8' }} />
                                      : msg.failed
                                        ? <span className="text-[10px] text-red-400">✕</span>
                                        : <span style={{ color: msg.isRead ? '#2176d8' : (isDark ? 'rgba(255,255,255,0.3)' : '#b0c0d8') }}>
                ...
                <div className="ia px-3 md:px-4 py-3 flex-shrink-0">
                  <form onSubmit={handleSendMessage} className="flex items-end gap-2 md:gap-3 max-w-4xl mx-auto">
                    <div className="ib flex-1 rounded-2xl flex items-end overflow-hidden">
                      <button type="button" className="p-3 shrink-0 hidden sm:block transition-colors"
                        style={{ color: isDark ? 'rgba(255,255,255,0.3)' : '#aab8d0' }}
                        onMouseEnter={e => (e.currentTarget.style.color = '#f59e0b')}
                        onMouseLeave={e => (e.currentTarget.style.color = isDark ? 'rgba(255,255,255,0.3)' : '#aab8d0')}>
                        <Smile size={19} />
                      </button>
                      <textarea
                        ref={textareaRef}
                        ...
                        style={{ color: isDark ? '#f1f5f9' : '#1a2540', lineHeight: '1.5' }}
                        rows={1}
                      />
                    </div>
                    <button type="submit" disabled={!messageText.trim() || isSending}
                      ...
                      {isSending
                        ...
                        : <Send size={19} style={{ marginLeft: messageText.trim() ? 1 : 0 }} />}
                    </button>
                  </form>
                </div>
              </>
